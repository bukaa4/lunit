...
public interface SlideRepository extends JpaRepository<Slide, Long> {
    ...
    Page<Slide> findAllSlidesByUserId(@Param("userId") Long userId,
                                      Pageable pageable);
    @Query("SELECT s FROM Slide s WHERE s.id = :slideId and s.userId = :userId")
    Optional<Slide> findSlideByUserIdAndSlideId(@Param("userId") Long userId,
                                                @Param("slideId") Long slideId);
    @Query("SELECT s FROM Slide s WHERE s.status != Status.NONE and s.userId = :userId")
    Page<Slide> findCompletedSlideByUserIdAndSlideId(@Param("userId") Long userId,
                                                     Pageable pageable);
